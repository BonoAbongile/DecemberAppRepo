...
namespace DecemberAppTask.ViewModels
{
    ...
    _updateLocationCommand ?? (_updateLocationCommand = new DelegateCommand(ExecuteUpdateLocations));
        ...
        private DelegateCommand _navigateToForm;
        public DelegateCommand NavigateToForm =>
            _navigateToForm ?? (_navigateToForm = new DelegateCommand(ExecuteNavigateToForm));

        private async void ExecuteNavigateToForm()
        {
            await NavigationService.NavigateAsync("ReportPage");
        }

        public LandingPageViewModel(INavigationService navigationService, IMapping mapping) : base(navigationService)
        {
